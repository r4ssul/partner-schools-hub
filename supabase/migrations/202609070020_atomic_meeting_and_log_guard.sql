-- Correct the conflict target in atomic meeting creation and retain the
-- password-setup guard on the developer's destructive log operation.

create or replace function public.create_workspace_item(target_workspace_id bigint, item jsonb)
...
begin
  ...
  if kind = 'folder' then
    ...
    insert into public.meeting_attendees(meeting_id, user_id, workspace_id)
      select result, a, target_workspace_id from unnest(attendees) a
      on conflict on constraint meeting_attendees_pkey do nothing;
...
begin
  if not public.has_completed_password_setup() or not exists (
  ...
  if target_scope is null or target_scope not in ('activity', 'members', 'all') then
