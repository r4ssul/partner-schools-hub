        ...
        <form className="creation-form" onSubmit={submit} noValidate aria-busy={isSubmitting}>
          ...
          <fieldset className="form-grid creation-fields" disabled={isSubmitting}>
            ...
            {kind === 'event' || kind === 'meeting' ? <>
              ...
              <fieldset className="choice-list field--full"><legend>Attendees</legend><p className="field-help">{kind === 'event' ? 'The event appears to everyone; selected attendees receive an assignment notification.' : 'The meeting appears to everyone; selected attendees receive an assignment notification. You are always included as its creator.'}</p>{data.members.filter((member) => member.active).map((member) => <label key={member.id}><input type="checkbox" value={member.id} disabled={kind === 'meeting' && member.id === currentUser.id} {...form.register('attendeeIds')} /><span><strong>{member.name}</strong><small>{member.organization}</small></span></label>)}</fieldset>
            </> : null}
            {kind === 'task' ? <>
