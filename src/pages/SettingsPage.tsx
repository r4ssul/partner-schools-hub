  ...
  return (
    <div className="page feature-page settings-page">
      ...
      <div className="settings-grid">
        ...
        <form className="content-surface settings-card" onSubmit={saveNotifications}><header><Bell size={22} /><div><h2>Notifications</h2><p>Choose where operational reminders are delivered.</p></div></header><label className="toggle-row"><span><strong>Email notifications</strong><small>Assignments and 24-hour reminders, subject to email-service availability.</small></span><input type="checkbox" checked={emails} onChange={(event) => setEmails(event.target.checked)} /></label><label className="toggle-row"><span><strong>In-app notifications</strong><small>Required for collaboration updates.</small></span><input type="checkbox" checked disabled /></label><button className="button button--primary settings-save-button">{savedSection === 'notifications' ? <><Check size={17} /> Saved</> : 'Save notifications'}</button></form>
