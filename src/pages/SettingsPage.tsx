...
export default function SettingsPage() {
  ...
  const saveWorkspace = async (event: React.FormEvent) => {
    event.preventDefault()
    await updateSettings(name, data.settings.emailNotifications)
    showSaved('workspace')
  }
  const saveNotifications = async (event: React.FormEvent) => {
    event.preventDefault()
    await updateSettings(data.settings.name, emails)
  ...
  return (
    <div className="page feature-page settings-page">
      ...
      <div className="settings-grid">
        ...
        <form className="content-surface settings-card" onSubmit={saveWorkspace}><header><Building2 size={22} /><div><h2>Workspace</h2><p>Basic details used throughout the portal.</p></div></header><label className="field"><span>Workspace name</span><input value={name} onChange={(event) => setName(event.target.value)} disabled={currentUser.role !== 'owner'} /></label><label className="field"><span>Workspace timezone</span><input value="Asia/Tokyo" disabled /></label><button className="button button--primary" disabled={currentUser.role !== 'owner'}>{savedSection === 'workspace' ? <><Check size={17} /> Saved</> : 'Save workspace'}</button></form>
