...
import { downloadFileFromR2, downloadFileFromSupabase } from '../lib/fileApi'
import { supabase } from '../lib/supabase'
import type { DocumentVersion, EntityKind, Folder as HubFolder, HubDocument } from '../types'
...
export default function FilesPage() {
  const { data, uploadFile, uploadUrls, addDocumentVersion, archiveItem } = useWorkspace()
  ...
  const openDocument = async (document: HubDocument, requestedVersion?: DocumentVersion) => {
    const localUrl = uploadUrls.get(document.id)
    if (localUrl) { window.open(localUrl, '_blank', 'noopener,noreferrer'); return }
    if (supabase) {
      const version = requestedVersion ?? document.versions.at(-1)
      if (!version) {
        setMessage('This file has no available versions.')
        return
      }
      ...
      const result = version.storageProvider === 'r2'
        ? await downloadFileFromR2(Number(version.id))
        : await downloadFileFromSupabase(Number(version.id))
