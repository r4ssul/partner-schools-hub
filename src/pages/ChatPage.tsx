...
export default function ChatPage() {
  ...
  return <div className="page feature-page chat-page">
    ...
    <div className="chat-page-layout">
      ...
      <aside className="content-surface chat-team"><h2><UsersRound size={19} /> In this conversation</h2><p>Everyone in the workspace can join in.</p>{members.map((member) => <div className="chat-team-member" key={member.id}><Avatar member={member} size="sm" /><div><strong>{member.name}{member.id === currentUser.id ? ' (you)' : ''}</strong><span>{member.organization}</span><small>{memberRoleLabel(member.role)}</small></div></div>)}<div className="chat-team-note"><LockKeyhole size={18} /><p>Messages are saved securely for your team. Use Files to share documents and Tasks for follow-ups.</p></div></aside>
    </div>
  </div>
}
