...
test('creates a shared event with Jan and a linked document', async ({ page }) => {
  ...
  await page.getByLabel('Attendees').selectOption({ label: 'Jan Baloglu' })
  await page.getByLabel('Linked files').selectOption({ label: 'sample.txt' })
...
test('searches newly created workspace content and starts with no notifications', async ({ page }) => {
  ...
  else await page.getByRole('tab', { name: 'Quick link' }).click()
