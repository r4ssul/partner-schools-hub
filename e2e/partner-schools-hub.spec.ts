...
test('Admins can view the active team but cannot manage membership', async ({ page }, testInfo) => {
  await page.evaluate((key) => {
    const value = JSON.parse(localStorage.getItem(key) || '{}')
    value.members[0].role = 'admin'
    value.members.push({ ...value.members[0], id: 'former-qa', name: 'Former QA', email: 'former@example.com', active: false })
    localStorage.setItem(key, JSON.stringify(value))
  }, STORAGE_KEY)
  await page.reload()
  await page.locator('.user-button').click()
  await page.getByRole('menuitem', { name: 'Team directory' }).click()
  await expect(page.getByRole('heading', { name: 'Team directory' })).toBeVisible()
  await expect(page.locator('.member-row')).toHaveCount(2)
  await expect(page.getByRole('button', { name: /Invite team member|Deactivate|Delete account|Former members/ })).toHaveCount(0)
  await expect(page.getByText('Former QA')).toHaveCount(0)
  await expect(page.locator('.security-copy')).toContainText('All other members are Admins and can manage shared content and use team chat.')
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  await page.screenshot({ path: `/private/tmp/partner-hub-team-${testInfo.project.name}.png`, scale: 'css', animations: 'disabled' })
...
test('developer can confirm former-account deletion and invite the same email again', async ({ page }, testInfo) => {
  await page.evaluate((key) => {
    const value = JSON.parse(localStorage.getItem(key) || '{}')
    value.members[0].canClearLogs = true
    value.members.push({ ...value.members[0], id: 'former-qa', name: 'Former QA', email: 'former@example.com', role: 'admin', active: false, canClearLogs: false })
    localStorage.setItem(key, JSON.stringify(value))
  }, STORAGE_KEY)
  await page.reload()
  await page.goto('/team')
  await page.getByRole('button', { name: 'Former members', exact: true }).click()
  const former = page.locator('.member-row').filter({ hasText: 'Former QA' })
  await former.getByRole('button', { name: 'Delete account' }).click()
  const dialog = page.getByRole('dialog', { name: 'Permanently delete this account?' })
  await expect(dialog.getByRole('button', { name: 'Delete permanently' })).toBeDisabled()
  await dialog.getByLabel('Type the member’s email to confirm').fill('wrong@example.com')
  await expect(dialog.getByRole('button', { name: 'Delete permanently' })).toBeDisabled()
  await page.screenshot({ path: `/private/tmp/partner-hub-delete-${testInfo.project.name}.png`, scale: 'css', animations: 'disabled' })
  await dialog.getByRole('button', { name: 'Cancel' }).click()
  await expect(former).toBeVisible()
  await former.getByRole('button', { name: 'Delete account' }).click()
  await dialog.getByLabel('Type the member’s email to confirm').fill('former@example.com')
  await dialog.getByRole('button', { name: 'Delete permanently' }).click()
  await expect(former).toHaveCount(0)
  await expect(page.getByRole('status')).toContainText('account was permanently deleted')
  await page.getByRole('button', { name: 'Invite team member' }).click()
  await page.getByLabel('Full name').fill('Former QA Returns')
  await page.getByLabel('Work email').fill('former@example.com')
  await page.getByLabel('Organisation').fill('QA School')
  await page.getByRole('button', { name: 'Send invitation' }).click()
  await expect(page.getByRole('status')).toContainText('Invitation sent to former@example.com')
  await page.getByRole('button', { name: 'Active team' }).click()
  await expect(page.locator('.member-row').filter({ hasText: 'Former QA Returns' })).toBeVisible()
})
